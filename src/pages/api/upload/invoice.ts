/**
 * Invoice File Upload API Endpoint
 * 
 * Handles file uploads for supplier invoices using local file storage.
 * Replaces Firebase Cloud Storage upload (requires Blaze plan).
 * 
 * Security:
 * - Requires authentication
 * - Role-based access control (Clerk, Manager, Administrator)
 * - File type validation (CSV, Excel, PDF only)
 * - File size limit (10MB max)
 * - Filename sanitization
 * 
 * Requirements: 10.1, 10.2, 19.1, 19.2
 */
...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    try {
      const filePath = await saveFile(file, 'invoices', supplierId);
